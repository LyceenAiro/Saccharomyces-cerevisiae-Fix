...
def plot_single(sg_index: int, _music_map: list, profile: list) -> str:
    ...
    yeast_anc = AnchorText(bg, 'yeast', 'Generated by Saccharomyces cerevisiae Fix by LyceenAiro', pen, yeast_font,
...
def plot_b50(_music_map: list, profile: list) -> str:
    ...
    yeast_anc = AnchorText(bg, 'yeast', 'Generated by Saccharomyces cerevisiae Fix by LyceenAiro',
...
def plot_summary(base_lv: int, _music_map: list, profile: list):
    ...
    """
    # Stipulate the size of the background & generate
    px_prologue, px_chapters, px_epilogue = 450, 0, 1950 #airo
    y_px, x_px = px_prologue + 1000 + px_epilogue, 1080
    bg = generate_hex_bg((y_px, x_px))
    ...
    yeast_anc = AnchorText(bg, 'yea', 'Generated by Saccharomyces cerevisiae Fix by LyceenAiro', pen, finale_font, (0, 0), finale_anc)
    author_anc = AnchorText(bg, 'author', 'Powered by Nyanm & Achernar Fix by LyceenAIro', pen, finale_font, (30, 0), finale_anc)
    yeast_anc.plot(color_gray, pos='c')
    author_anc.plot(color_gray, pos='c')

    text_layer = np.array(text_layer)
    png_superimpose(bg, text_layer)
    output_path = '%s/SMplus.png' % (cfg.output)
    cv2.imwrite(output_path, bg[:, :, :3], params=[cv2.IMWRITE_PNG_COMPRESSION, 3])
